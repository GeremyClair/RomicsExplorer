...
PRELOAD_FILE <- file.path(SCRIPT_DIR, "romics_proteins.rda")
PRELOAD_ID_TYPE <- "Gene"
PRELOAD_FACTOR <- "growth_media"
...
ui <- dashboardPage(skin="black",
  ...
  dashboardBody(useShinyjs(),
    tags$script(HTML("$(document).ready(function(){setTimeout(function(){$('ul.sidebar-menu li').each(function(){var a=$(this).find('> a');var v=a.attr('data-value');if(v==='ontology'||v==='enrichment')$(this).hide();});},200);});
    ...
    tabItems(
      tabItem(tabName="welcome",
        fluidRow(box(title="Dataset Description", status="info", solidHeader=TRUE, width=12,
          p("The Romics_object that was loaded represent a mass-spectrometry based dataset of B. cereus grown in four different media conditions harvested in mid-exponential phase and analyzed in Data-Dependent Acquisition (DDA) mode (n=3 per condition).", style="font-size:14px; line-height:1.6;")
        )),
        fluidRow(box(title="Dataset Configuration", status="primary", solidHeader=TRUE, width=12,
          fluidRow(
            column(3, h5("ID Type:"), selectInput("welcome_id_select", "", choices=character(0)), actionButton("welcome_apply_id", "Apply", class="btn-success", width="100%")),
            column(3, h5("Factor:"), selectInput("welcome_factor_select", "", choices=character(0)), actionButton("welcome_apply_factor", "Apply", class="btn-success", width="100%")),
            column(6, h5("Dataset Info:"), verbatimTextOutput("welcome_info"))
          )
        )),
        fluidRow(box(title="Distribution - Boxplot", status="info", solidHeader=TRUE, width=12,
          plotlyOutput("welcome_boxplot", height="700px")
        ))),
...
server <- function(input, output, session) {
  ...
  # Autoload preloaded file
  observe({
    if (is.null(romics_object())) {
      session$onFlushed(function() {
        tryCatch({
          if (file.exists(PRELOAD_FILE)) {
            env <- new.env()
            load(PRELOAD_FILE, envir = env)
            obj <- env[[ls(env)[1]]]
            if (!is.null(PRELOAD_ID_TYPE)) {
              obj <- RomicsProcessor::romicsChangeIDs(romics_object = obj, newIDs = PRELOAD_ID_TYPE)
            }
            romics_object(obj)
            if (!is.null(PRELOAD_FACTOR)) current_factor(PRELOAD_FACTOR)
            # Update feature lists for all tabs
            ftn <- rownames(obj$data)
            if (length(ftn) > 0) {
              updateSelectizeInput(session, "feature_select", choices=ftn, selected=ftn[1])
              updateSelectizeInput(session, "multi_feature_dropdown", choices=ftn)
              updateSelectizeInput(session, "grouping_feature_select", choices=ftn)
            }
            showNotification(paste("Loaded:", basename(PRELOAD_FILE)), type = "message", duration = 3)
          }
        }, error = function(e) {})
      }, once = TRUE)
    }
  ...
  # Welcome tab info
  output$welcome_info <- renderPrint({
    obj <- romics_object()
    if (is.null(obj)) {
      cat("Waiting for dataset...\n")
    } else {
      cat("Samples:", ncol(obj$data), "\n")
      cat("Features:", nrow(obj$data), "\n")
    }
  })
  
  # Welcome tab boxplot
  welcome_plot_data <- reactive({
    obj <- romics_object()
    if (is.null(obj)) return(NULL)
    tryCatch({
      ggplotly(RomicsProcessor::distribBoxplot(obj))
    }, error = function(e) NULL)
  })
  
  output$welcome_boxplot <- renderPlotly({
    p <- welcome_plot_data()
    if (is.null(p)) plotly_empty() else p
  })
  
  # Welcome: Update ID select
  observe({
    req(romics_object())
    if (!is.null(romics_object()$IDs)) {
      choices <- colnames(romics_object()$IDs)
      updateSelectInput(session, "welcome_id_select", choices=choices, selected=choices[1])
    }
  })
  
  # Welcome: Update factor select
  observe({
    req(romics_object())
    fns <- get_clean_factors(romics_object())
    if (length(fns) > 0) {
      updateSelectInput(session, "welcome_factor_select", choices=fns, selected=current_factor())
    }
  })
  
  # Welcome: Apply ID
  observeEvent(input$welcome_apply_id, {
    req(romics_object(), input$welcome_id_select)
    tryCatch({
      obj <- RomicsProcessor::romicsChangeIDs(romics_object=romics_object(), newIDs=input$welcome_id_select)
      romics_object(obj)
      showNotification(paste("ID:", input$welcome_id_select), type="message")
    }, error=function(e) showNotification(paste("Error:", e$message), type="error"))
  })
  
  # Welcome: Apply factor
  observeEvent(input$welcome_apply_factor, {
    req(input$welcome_factor_select)
    current_factor(input$welcome_factor_select)
  })
